...
# Load spaCy model (if not already loaded)
try:
    # Try to load a larger model with word vectors if available
    try:
        nlp = spacy.load("en_core_web_md")
        logger.info("Loaded spaCy model with word vectors (en_core_web_md)")
    except:
        # Fall back to smaller model
        nlp = spacy.load("en_core_web_sm")
        logger.warning("Using small spaCy model (en_core_web_sm) without word vectors. Similarity calculations will be less accurate.")
except:
    # If model not installed, download it
...
class DocumentProcessor:
    ...
    def create_document_embeddings(self):
        ...
        # Check if we have enough documents for min_df=2 to work
        if len(docs_content) < 2:
            # Adjust min_df for a single document
            self.vectorizer.min_df = 1
            logger.warning("Adjusted min_df to 1 because there is only one document.")
...
class CustomSimilarityCalculator:
    """Custom similarity calculator that doesn't rely on word vectors."""
...
class SummaryGenerator:
    ...
    def __init__(self):
        ...
        # Initialize custom similarity calculator
        self.similarity_calculator = CustomSimilarityCalculator()
    ...
    def _cluster_related_sentences(self, sentences: List[str]) -> List[List[str]]:
        ...
        for i in range(len(sentences)):
            for j in range(i+1, len(sentences)):
                # Use custom similarity calculator
                similarity = self.similarity_calculator.calculate_similarity(sentences[i], sentences[j])
                similarity_matrix[i, j] = similarity
                similarity_matrix[j, i] = similarity
        ...
        for i in range(len(sentences)):
            ...
            # Find similar sentences
            for j in range(len(sentences)):
                if j != i and j not in used_indices and similarity_matrix[i, j] > 0.3:
                    cluster.append(sentences[j])
                    used_indices.add(j)
